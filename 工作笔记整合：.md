...
2. 当直接编写service实现类时，spring则会默认使用CGLIB代理，并完成正常的依赖注入
...
默认情况下，编译器通过xml文件中指定的url，实时获取其文档定义文件的内容，然后进行内容验证，因此当无网络环境下，无法对xml文件进行正确的内容验证和标签提示（**但并不会影响java库对xml元数据的读取**）
