...
![](C:\Users\OneMTime\Desktop\Typora图片\20180629153736223.jfif)
...
12、servlet原生获取POST请求中body数据（非from data，对于springMVC直接使用@bodyrequest注解）：
...
7、oracle中select * 时，不能继续再写出其他字段，即使是rownum；因此当使用rownum进行指定第3-6行数据查询时，需要使用t.*，rownum 来指定rownum字段；然后作为子查询表进一步对原表的rownum进行条件过滤
...
JSONArray 转JSONObject时，推荐使用for循环来遍历 ，jsonArray.getJSONObject(i)直接获取jsonObject对象；
若使用foreach，则需要默认指定取出的对象为Object，此时就需要多一步JSONObject.fromObject(Object)的转化
...
​	1、JSONObjcet和hashMap的区别： 都允许value为null；但JSONObjcet不允许key为null

​	2、JSONObject.get(KEY),当key不存在时，返回null不会报错；值为null时，返回null

​	3、	使用jsonobject.getString(key)、jsonobject.getInt(key)等方法，key不存在或者key对应value为null时，会报错key没有找到
...
- 将class文件在jvm上允许；当该类需要被使用时（实例化、使用类的静态成员或静态方法），就将其加载到内存中
...
1、Java内部使用Unicode字符集来表达字符；当编译成为Class文件时，使用utf-8来将其进行二进制存储

2、javaString进行编码、解码时，默认使用该项目入口函数文件的编码格式，可以通过System.getProperty("file.encoding")查看；

**java进行字节转变字符时，默认使用项目默认编码（System.getProperty("file.encoding")）；由于UTF-8、GBK都兼容ISO-8858-1，因此对于西欧字符都能正常转化**

但是为了保证项目的可移植性，防止源文件编码格式被随意更改，再编译，导致程序内部默认编码格式改变。因此，一般情况下，使用显式的方式对字符串进行编码、解码

3、在文本文件使用java io流读取时，字节流FileInputStream转化为字符流InputStreamReader，需要其编码格式与文件文本编码格式一致；默认为System.getProperty("file.encoding")

​		对于FileReader/FileWriter字符包装流，直接是使用默认的javaString编码格式，无法自己指定

4、java控制台打印默认使用操作系统默认编码（中文操作系统，默认为GBK），Elipse可以手动设置java -jar的执行命令行参数，从而改变打印的编码格式

cmd中使用该  java  -Dfile.encoding=utf-8  -jar xxxx.jar 命令行来允许jar包
...
- 参数列表中的类型可以省略，但要保证参数顺序一 一对应（否则会出现检查时异常）
...
- 场景使用场景：
  ...
  Arrays.sort/Collections.sort方法：它需要两个参数，数组/有序集合和Comparator（比较器，为一个函数式接口）
  ...
  对于Comparator接口，就可以通过lambda表达式进行实例化实现：

  ```java
  		ArrayList<Account> arrayList = new ArrayList<Account>();
  		
  		Collections.sort(arrayList,new Comparator<Account>() {
  			@Override
  		...
  		Collections.sort(arrayList,(a,b)->a.getAccount().compareTo(b.getAccount()));
...
3、并且两者文件不是完全相同（java8之前），专用JRE使用的client VM（客户端java虚拟机）（JAVA8专用jre使用的是 server VM）；公用JRE使用的是 server VM（服务端java虚拟机），启动时间长、内存消耗多，但执行性能高。Nginx
