...
​	以Hotspot版本的JVM为例，元空间出现在JDK8中，在JDK7及之前版本，元空间的前身为Perm区，又叫永久代，是一个单独的内存空间，用于存放类的一些元数据（类的静态变量、final类型变量、方法信息、类信息、类常量值）。由于perm区大小在JVM启动时就固定，并且只有当触发FullCG时，才会进行垃圾回收清理，因此在类加载过多时，会导致OOM（**即类创建实例很少，但加载很多，导致堆区没有占满的情况下，perm区被占满，在web项目中，由于功能点比较多，需要动态加载很多类，因此比较常出现**），对于这种情况，只能将perm区设置大一点，这样也就比较难调优，浪费内存分配额，并且当项目转移时，新开发人员由于不知道之前的perm区大小设置，而浪费大量时间分析bug(oom原因)

​	在JDK8中，使用元空间来代替永久代，做出了如下改变：

1. 将String类型常量转移到了堆区保存
2. 将类元信息、字段、？？？？？？
