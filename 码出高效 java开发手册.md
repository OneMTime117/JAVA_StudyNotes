...
3. **Parallel回收器（分为新生代和老年代Parallel Old ）：**
   ...
   特点：**多线程并行收集，存在STW，但可以手动优化控制最大停顿时间的前提下，提高吞吐量（cpu运行程序系统的时间，减少GC回收时间占比，最大化提高系统性能），适合使用在吞吐量优先需求中**
...
5. **G1回收器：**
   ...
   特点：**多线程并行、并发收集，独特的空间划分不会产生空间碎片，但需要更大的内存空间；可以预测GC的STW时间，从而在有限时间内，选择最高效率的region进行优先处理；内部还是使用分代收集，但不在内存空间中体现**
...
因此java基本使用选择有: ParNew/CMS（响应更快，但吞吐量低，浪费内存空间）、Parallel/Parallel Old（STW可控,吞吐量高）、Parallel/serial Old（中庸，多线程分代使用最合适的算法，没有额外特点）、G1（STW可控、吞吐量高，但耗费内存空间大）
